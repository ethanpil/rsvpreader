...
if (window.rsvpInstance) {
  ...
  window.rsvpInstance = null; // Ensure it's fully cleared before reassigning
}
...
class RSVPReader {
  ...
  remove() {
      ...
      // No need to check window.rsvpInstance === this here, 
      // because this method is called ON the instance.
      // The check before creating a new instance handles clearing the global var.
...
// --- Initialize ---
// This part is already handled by the check at the top of the file.
// If an old instance exists, it's removed. Then a new one is created.
try {
    // The check for window.rsvpInstance and its removal is now at the top.
